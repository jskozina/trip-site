...
**stay** (inside a leg)
`id, title, subtitle, start, end, lat, lng, priceAud, url, image, images, body, status`
...
- `priceAud` on a **stay** is per night, a whole number, no currency symbol — the site multiplies by nights. On a **spot** it's a flat one-off cost (a ticket, a meal). Stay and spot prices are the shared/total cost for the whole group, not per person. On a **transit** it's different: for a **flight**, `priceAud` is the *per-person* fare — the site multiplies by `trip.travellers` when showing the "Together" total. For a train, bus, car or ferry, it's the flat shared cost same as a spot (no multiplying by travellers). The budget section has a Together/Per person toggle so the human can view it either way — this split is what makes that toggle correct, so don't put a per-person flight fare in as if it were a shared total or vice versa. For a connecting flight (segments sharing a `group`), put the whole fare on one segment only, never split or repeat it across segments, or it'll double-count in the budget.
...
- `images` is optional on a **leg**, where it means something different than on a **stay**. On a leg, it's an array of photo URLs the calendar cycles through, one per day of that city's stay (day 1 gets `images[0]`, day 2 gets `images[1]`, wrapping around), so the "at a glance" view shows real variety instead of one photo repeated every night. On a **stay**, it's a small swipeable gallery on that stay's card (a handful of real photos of the actual place, once it's booked) - a nice-to-have, not required. Both fall back to the single `image` if `images` isn't set, and `image` should equal `images[0]` so the fallback and the first photo shown always match.
